...
(async () => {
    ...
    try {
        ...
        console.log(
            colors.red('✘ Unable to locate ./app/data.csv file. See README.md. Aborting...\n')
        );
        process.exit(1);
    }
    parseCSV(csv, { columns: true }, async (err, output) => {
        if (err) {
            console.error(
                colors.red.bold('✖︎ Error parsing data.csv. Please retry. Aborting...\n')
            );
        ...
        try {
            ...
            const bar = new ProgressBar(
                colors.magenta.bold('★ :current / :total :bar :percent (:elapseds)'),
                { total: output.length, width: 75 }
            );
            ...
            for (const entry of output) {
                if (!('ItemID' in entry) || !('Stock' in entry) || !('Price' in entry)) {
                    ...
                    ekmClient.logError({
                        entry,
                        err: 'CSV entry is missing ItemID, Stock or Price',
                    });
                } else {
                    await ekmClient.updateProduct(
                        entry.ItemID,
                        entry.Stock,
                        entry.Price.replace('£', '')
                    );
                ...
                if (bar.complete) {
                    ...
                    fs.writeFileSync(
                        path.resolve('reports', tag).replace(/\\/g, '/'),
                        JSON.stringify({ errors, updates }, null, 4)
                    );
